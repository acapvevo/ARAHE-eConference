                ...
                <form class="user" method="POST" action="{{ route('super_admin.password.update') }}">
                    ...
                    <div class="mb-3">
                        <input class="form-control form-control-user {{ $errors->has('email') ? 'is-invalid' : '' }}"
                            type="email" id="email" placeholder="Enter Email Address" name="email" required value="{{old('email', $request->email)}}">
                        @error('email')
                            <div class="invalid-feedback">
                                {{ $message }}
                            </div>
                    ...
                    <div class="mb-3">
                        <input class="form-control form-control-user {{ $errors->has('password_confirmation') ? 'is-invalid' : '' }}"
                            type="password" id="password_confirmation" placeholder="Enter Password Again" name="password_confirmation" required
